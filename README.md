...
*   **Unified lead pipeline (`src/lib/leads`, `src/app/actions/leads.ts`):** every form (pre-approval, contact, valuation expert opinion, exit popup) calls `createLead`, which validates, stores one document in the `leads` Firestore collection via the Admin SDK, emails Ken with `geoff.radian6@gmail.com` BCC'd, emails the lead a confirmation, texts Ken and the lead when Twilio is configured, and schedules a 48-hour "did Ken reach you?" check-in (cancelled when Ken marks the lead Contacted in /ken). Check-in replies (one-click email links or SMS YES/NO via `/api/twilio/inbound`) are recorded on the lead; a "no" nudges Ken immediately.
*   **Hourly automation (`/api/cron/leads`, called by `.github/workflows/lead-automation.yml`):** sends due check-ins during Toronto daytime and a daily 8am digest (new leads, untouched leads older than one business day, "no" replies, follow-ups due) to Geoff and Ken. Secured by the `CRON_SECRET` secret (Secret Manager + GitHub secret).
*   **Lead inbox at `/ken`:** every lead with status, notes, next follow-up date, check-in result and delivery errors; "Needs attention" filter surfaces leads Ken has not touched.
